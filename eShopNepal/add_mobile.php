<?php
	...
	if(isset($_GET['product_id'])){
		...
		$getData = mysqli_query($con,"select * from product_mobile where prod_id ='$id'");
			...
			if($id)
			{
				...
				$prod_no=$_POST['txtProdNo'];
					...
					if(move_uploaded_file($_FILES['txtProdImage']['tmp_name'], $target)) {
						$sql = "UPDATE `product_mobile` SET `prod_no`='$prod_no', `prod_name`='$prod_name',`prod_image`='$image_name',`prod_brandname`='$prod_brandname' ,`prod_price`='$prod_price', `prod_quan`='$prod_quan', `date_added`=now(), `keyword`='$prod_keyword',`category`='product_mobile'  WHERE `prod_id`='$id'";
				...
				$sqlSelExistNO= mysqli_query($con,"select * from product_mobile where prod_id = '$_POST[txtProdNo]'");
				if(mysqli_num_rows($sqlSelExistNO) >= 1)
					$msg = "Product ID is already in use!";
				else{
					$prod_no=$_POST['txtProdNo'];
					$prod_name=$_POST['txtProdName'];
					$prod_brandname=$_POST['txtProdBrand'];
					$prod_price=$_POST['txtProdPrice'];
					$prod_quan=$_POST['txtProdQuan'];
					$prod_keyword=$_POST['txtProdKeyword'];
					$image = file_get_contents ($_FILES['txtProdImage']['tmp_name']);
					...
					if(move_uploaded_file($_FILES['txtProdImage']['tmp_name'], $target)) {
						$sql = "INSERT INTO `product_mobile`(`prod_id`, `prod_no`, `prod_name`, `prod_brandname`,`prod_image`, `prod_price`, `prod_quan`, `date_added`, `keyword`,`category`) VALUES (NULL,'$prod_no','$prod_name','$prod_brandname','$image_name','$prod_price','$prod_quan',now(),'$prod_keyword','product_mobile')";
...
<html>
    ...
    <body>
		...
		<div id="admin_content">
			<h2>MOBILE PRODUCT INFORMATION<span style="color: #a11; font-size: 13px; margin-left: 50px;"><?php echo $msg; ?><span></h2>
			...
			<table>
				...
				<tr>
					...
					<td><input  class = "input_form"  type="text" name="txtProdBrand" value="<?php echo $prod_brandname ?>"/></td>
				</tr>
				<tr>
					<td>Product Price:</td>
