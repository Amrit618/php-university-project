<?php
	...
	if(isset($_GET['product_id'])){
		...
		$getData = mysqli_query($con,"select * from product_men where prod_id ='$id'");
			...
			if($id)
			{
				...
				$prod_no=$_POST['txtProdNo'];
					...
					if(move_uploaded_file($_FILES['txtProdImage']['tmp_name'], $target)) {
						$sql = "UPDATE `product_men` SET `prod_no`='$prod_no', `prod_name`='$prod_name',`prod_image`='$image_name', `prod_price`='$prod_price', `prod_quan`='$prod_quan', `date_added`=now(), `keyword`='$prod_keyword',`category`='product_men'  WHERE `prod_id`='$id'";
				...
				$sqlSelExistNO= mysqli_query($con,"select * from product_men where prod_id = '$_POST[txtProdNo]'");
				if(mysqli_num_rows($sqlSelExistNO) >= 1)
					...
					if(move_uploaded_file($_FILES['txtProdImage']['tmp_name'], $target)) {
						$sql = "INSERT INTO `product_men`(`prod_id`, `prod_no`, `prod_name`,`prod_image`, `prod_price`, `prod_quan`, `date_added`, `keyword`,`category`) VALUES (NULL,'$prod_no','$prod_name','$image_name','$prod_price','$prod_quan',now(),'$prod_keyword','product_men')";
...
<html>
    ...
    <body>
		...
		<div id="admin_content">
			<h2>MENS PRODUCT INFROMATION<span style="color: #a11; font-size: 13px; margin-left: 50px;"><?php echo $msg; ?><span></h2>
